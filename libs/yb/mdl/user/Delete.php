...
/**
 * YakiBiki user module delete page
 *
...
class yb_mdl_user_Delete
{
    ...
    function confirm_page(&$runner, $page, &$bookmark, $params)
    {
        ...
        $renderer->setTitle(t('User Delete') . ' (' . t('Confirm') . ')');


        $renderer->set('form', $form_datas);
        $renderer->set('virtual', $virtual);

        return "theme:modules/user/delete_confirm_tpl.html";
    }
    ...
    function on_save(&$runner, $event, &$bookmark, $params)
    {
        ...
        yb_tx_user_Delete::go($id);

        $log =& yb_Log::get_Logger();
        $log->info(sprintf("user delete success: id=%d, name=%s", 
            $id, $virtual['name']));
    ...
    function finish_page(&$runner, $page, &$bookmark, $params)
    {
        ...
        $renderer =& $runner->getRenderer();
        $renderer->setTitle(t('User Delete') . ' (' . t('Complete') . ')');
        $renderer->set('virtual', $virtual);
        $renderer->set('form', $form_datas);

        return "theme:modules/user/delete_finish_tpl.html";
    }
